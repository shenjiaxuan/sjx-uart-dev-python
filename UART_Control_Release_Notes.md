...
## 版本 3.3.1 - 2026年01月20日

### 主要更新
- **新增 LTE 开关控制**: 实现 LTE 硬件开关功能
  - 新增 `CELL|` 命令，支持查询和控制 LTE 硬件状态
  - 支持通过 SDK 中间件设置 LTE 模块状态（AT+CFUN命令）
  - LTE 状态修改自动同步到 gs501.json 配置文件
  - 修改 LTE 状态会自动 kill main 进程以应用新配置
  - 查询命令：`CELL|` - 返回当前 LTE 开关状态
  - 设置命令：`CELL|0` (关闭) 或 `CELL|1` (开启)
  - 响应格式：`{"CellularEnable": 0/1}`

- **新增 WiFi 密码修改功能**: 支持通过 UART 动态修改 WiFi AP 密码
  - 新增 `WFPW|` 命令，支持查询和设置 WiFi AP 密码
  - 密码采用 Base64 编码传输，确保特殊字符安全传输
  - 支持 WPA2 密码长度验证（8-63字符）
  - 密码修改通过 SDK 中间件调用 Python 脚本完成
  - 支持 AS_8MP (hostapd) 和其他板子 (NetworkManager) 两种方式
  - 修改密码后自动重启 WiFi AP 应用新密码
  - 查询命令：`WFPW|` - 返回当前密码的 Base64 编码
  - 设置命令：`WFPW|<base64_password>` - 设置新密码
  - 响应格式：成功时返回 `{"Password": "<base64_password>"}`，失败时返回 `{"Password": ""}`

- **?PSxx 图像块命令范围固定为最大值 80**: 优化 `?PSxx` 命令的有效范围
  - `?PS5` 到 `?PS84` 始终有效，不再随 `BLK|` 配置动态变化
  - `BLK|` 仍然影响图像压缩质量和目标大小，但不限制 `?PSxx` 的可查询范围
  - 超出实际图像块数量但在 80 范围内的请求返回空包 `{"BlockN":""}`

### 技术改进
- **SDK 集成增强**: 新增 LTE 硬件控制和 WiFi 密码管理 SDK API 对接
- **错误处理优化**: WiFi 密码修改失败时仅返回空密码，简化错误响应格式
- **配置同步**: LTE 开关状态自动同步到 sdk_config.json 和 gs501.json
- **硬件兼容性**: WiFi 密码修改支持不同硬件平台的 AP 管理方式
- **命令范围统一**: `?PSxx` 有效范围固定为硬件最大支持值，提高命令行为一致性

---

## 版本 3.2.4 - 2025年12月12日

### 主要更新
- **新增 ?RST 指令**: 实现系统重置功能
  - 新增 `?RST` 查询命令，支持系统状态重置
  - 提供快速恢复系统初始状态的能力
  - 增强系统故障恢复和维护能力
...
- **WiFi密码管理**: 支持动态修改WiFi AP密码，密码采用Base64编码传输
- **LTE硬件控制**: 支持LTE模块开关控制，通过AT命令控制硬件状态
...
| `WFPW\|` | 查询当前WiFi密码 | {"Password": "base64编码的密码"} |
| `CELL\|` | 查询LTE硬件状态 | {"CellularEnable": 0/1} |
| `?RST` | 系统重置 | 重置确认信息 |
...
| `WFPW\|[密码]` | 设置WiFi密码 | Base64编码，8-63字符 |
| `CELL\|[状态]` | LTE硬件开关控制 | 0(关闭), 1(开启) |
| `REACT\|` | 查询紧急模式状态 | - |
